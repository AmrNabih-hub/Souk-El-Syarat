...
exports.onUserStatusChange = exports.initializeRealtime = exports.cleanupSessions = exports.dailyAnalytics = exports.api = void 0;
...
app.post('/api/auth/signin', async (req, res) => {
    try {
        ...
        res.json({
            ...
            role: (userData === null || userData === void 0 ? void 0 : userData.role) || 'customer',
...
app.post('/api/orders', async (req, res) => {
    try {
        ...
        const token = authHeader.split(' ')[1];
        const decodedToken = await admin.auth().verifyIdToken(token);
        const order = Object.assign(Object.assign({}, req.body), { customerId: decodedToken.uid, status: 'pending', createdAt: admin.firestore.FieldValue.serverTimestamp(), updatedAt: admin.firestore.FieldValue.serverTimestamp() });
...
app.post('/api/vendor/apply', async (req, res) => {
    try {
        ...
        const token = authHeader.split(' ')[1];
        const decodedToken = await admin.auth().verifyIdToken(token);
        const application = Object.assign(Object.assign({ userId: decodedToken.uid }, req.body), { status: 'pending', createdAt: admin.firestore.FieldValue.serverTimestamp() });
...
exports.dailyAnalytics = functions.pubsub.schedule('0 0 * * *').onRun(async (context) => {
    try {
...
exports.cleanupSessions = functions.pubsub.schedule('every 1 hours').onRun(async (context) => {
    try {
        const now = new Date();
...
exports.onUserStatusChange = functions.database.ref('/status/{uid}').onUpdate(async (change, context) => {
