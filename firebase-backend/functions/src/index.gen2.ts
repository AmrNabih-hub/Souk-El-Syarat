...
const authenticateUser = async (req: Request, res: Response, next: Function) => {
  try {
    ...
    (req as any).userRole = userDoc.data()?.role || 'customer';
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await db.collection('users').doc(userId).update({
        role: newRole,
        promotedBy: (req as any).user.uid,
...
app.get('/api/moderation/reports',
  authenticateUser,
  requirePermission('reports.handle'),
  async (req: Request, res: Response) => {
    try {
      ...
      const reportData = reports.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
...
app.post('/api/moderation/suspend-user',
  authenticateUser,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await db.collection('users').doc(userId).update({
        ...
        suspendedBy: (req as any).user.uid
      });
...
app.post('/api/inspector/verify-car',
  authenticateUser,
  requirePermission('cars.inspect'),
  auditLog,
  async (req: Request, res: Response) => {
    try {
      ...
      const inspectionRef = await db.collection('inspections').add({
        listingId,
        inspectorId: (req as any).user.uid,
      ...
      await db.collection('car_listings').doc(listingId).update({
        verified: true,
        verifiedBy: (req as any).user.uid,
...
app.put('/api/vendors/:vendorId/commission',
  authenticateUser,
  requirePermission('commissions.set'),
  auditLog,
  async (req: Request, res: Response) => {
    try {
      ...
      await db.collection('vendors').doc(vendorId).update({
        commissionRate,
        commissionUpdatedBy: (req as any).user.uid,
...
app.get('/api/premium/deals',
  authenticateUser,
  requirePermission('premium_deals.access'),
  async (req: Request, res: Response) => {
    try {
      ...
      const dealData = deals.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
...
app.post('/api/vendor/products/bulk',
  authenticateUser,
  requirePermission('products.bulk'),
  rateLimitByRole(),
  async (req: Request, res: Response) => {
    try {
...
export const processAsyncTask = onMessagePublished(
  {
    topic: 'async-tasks',
    region: 'us-central1',
    memory: '512MiB',
